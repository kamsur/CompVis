"""A demo CustomDataset class
for debugging dataset object of infant vision project"""
...
class CustomDataset(Dataset):
    ...
    def get_image_paths(self, *, data_path: str = "", data_type: str = "JPEG"):
        ...
        for root, _, files in os.walk(data_path):
            for file_name in files:
                if file_name.endswith(data_type.upper()) or file_name.endswith(
                    data_type.lower()
                ):
    ...
    def __getitem__(self, index):
        ...
        original_img = imread(Path(filename))
        ...
        original_img = torch.from_numpy(np.transpose(original_img, (2, 0, 1)))
        transformer = self.transform
        img = transformer(img)
        original_img = transformer(original_img)
        return (original_img, img, category)
